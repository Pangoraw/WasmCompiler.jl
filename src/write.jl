...
wwrite(io::IO, ::WasmInt32) = write(io, 0x7E)
...
wwrite(io::IO, lg::local_get) = wwrite(io, 0x20, lg.n)
wwrite(io::IO, c::call) = wwrite(io, 0x10, c.func)
...
function wwrite(io::IO, wmod::WModule)
    ...
    # 1. Type Section
    fntypes::Vector{FuncType} = unique(f -> f.fntype, wmod.funcs)
    sio = IOBuffer()
    wwrite(sio, fntypes)
    buf = take!(sio)
    n += write(io, 0x01)
    ...
    # 3. Func Section
    fntype_indices = map(f -> UInt32(findfirst(==(f.fntype), fntypes)), wmod.funcs)
    n += write(io, 0x03)
    n += wwrite(io, UInt32(sizeof(UInt32) * length(fntype_indices)))
    n += wwrite(io, fntype_indices)
