...
wwrite(io::IO, ::i32_trunc_sat_f32_s) = wwrite(io, 0xfc, UInt32(0))
wwrite(io::IO, ::i32_trunc_sat_f32_u) = wwrite(io, 0xfc, UInt32(1))
wwrite(io::IO, ::i32_trunc_sat_f64_s) = wwrite(io, 0xfc, UInt32(2))
wwrite(io::IO, ::i32_trunc_sat_f64_u) = wwrite(io, 0xfc, UInt32(3))
wwrite(io::IO, ::i64_trunc_sat_f32_s) = wwrite(io, 0xfc, UInt32(4))
wwrite(io::IO, ::i64_trunc_sat_f32_u) = wwrite(io, 0xfc, UInt32(5))
wwrite(io::IO, ::i64_trunc_sat_f64_s) = wwrite(io, 0xfc, UInt32(6))
wwrite(io::IO, ::i64_trunc_sat_f64_u) = wwrite(io, 0xfc, UInt32(7))
...
wwrite(io::IO, an::array_new_default) = wwrite(io, 0xfb, UInt32(7), an.typeidx - one(Index))
wwrite(io::IO, an::array_new_fixed) = wwrite(io, 0xfb, UInt32(8), an.typeidx - one(Index), an.length)
wwrite(io::IO, an::array_get) = wwrite(io, 0xfb, UInt32(11), an.typeidx - one(Index))
wwrite(io::IO, an::array_get_s) = wwrite(io, 0xfb, UInt32(12), an.typeidx - one(Index))
wwrite(io::IO, an::array_get_u) = wwrite(io, 0xfb, UInt32(13), an.typeidx - one(Index))
wwrite(io::IO, an::array_set) = wwrite(io, 0xfb, UInt32(14), an.typeidx - one(Index))
wwrite(io::IO, ::array_len) = wwrite(io, 0xfb, UInt32(15))
wwrite(io::IO, af::array_fill) = wwrite(io, 0xfb, UInt32(16), af.typeidx - one(Index))
wwrite(io::IO, ac::array_copy) = wwrite(io, 0xfb, UInt32(17), ac.src - one(Index), ac.tgt - one(Index))
